...
exports.createProperty = async (req, res) => {
  try {
    const { title, status, type, price, area, bedrooms, bathrooms, location, detailedInformation, contactInformation } = req.body;
    let images = [];
...
exports.incrementVeiws = async (req, res) => {
  try {
    let property = await Property.findByIdAndUpdate(req.params.id, { $inc: { views: 1 } }, { new: true });
    if (!property) {
